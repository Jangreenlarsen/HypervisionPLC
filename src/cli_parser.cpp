...
static void print_show_help(void) {
  ...
  debug_println("    show counters          - Alle counters");
  debug_println("    show counter <id> [verbose] - Specifik counter (1-4)");
  debug_println("    show timers            - Alle timers");
  debug_println("    show timer <id> [verbose] - Specifik timer (1-4)");
  ...
  debug_println("    show metrics           - Prometheus metrics reference");
  debug_println("    show watchdog          - Watchdog monitor status");
  debug_println("    show debug             - Debug flags");
  debug_println("    show echo              - Echo status");
  debug_println("");
}
...
static void print_set_help(void) {
  ...
  debug_println("    set hostname <name>       - Saet hostname");
  debug_println("    set echo <on|off>         - Saet remote echo");
  debug_println("    set gpio ?                - GPIO pin-mapping kommandoer");
  debug_println("");
  debug_println("  Modbus:");
  debug_println("    set modbus mode <slave|master|off> - Transceiver mode");
  debug_println("    set modbus-slave ?        - Slave config (id, baud, parity, ...)");
  debug_println("    set modbus-master ?       - Master config (baud, timeout, ...)");
  debug_println("    set reg <addr> <value>    - Skriv holding register");
  debug_println("    set coil <idx> <0|1>      - Skriv coil");
  debug_println("");
  debug_println("  Features:");
  debug_println("    set counter ?             - Counter kommandoer");
  debug_println("    set timer ?               - Timer kommandoer");
  debug_println("    set logic ?               - ST Logic kommandoer");
  debug_println("    set persist ?             - Persistence grupper");
  debug_println("");
  debug_println("  Network:");
  debug_println("    set wifi ?                - Wi-Fi kommandoer");
  debug_println("    set telnet ?              - Telnet kommandoer");
  debug_println("    set ethernet ?            - Ethernet (W5500) kommandoer");
  ...
  debug_println("    set baud <rate>           - -> set modbus-slave baudrate <rate>");
  debug_println("    set id <slave_id>         - -> set modbus-slave slave-id <id>");
  debug_println("");
}
  ...
  if (!strcmp(cmd, "SHOW")) {
    ...
    if (!strcmp(what, "CONFIG")) {
      ...
      if (argc >= 3 && !strcmp(normalize_alias(argv[2]), "ROLES")) {
        ...
        debug_println("");
        debug_println("Eksempel:");
        debug_println("  set user api_user password ChangeMe123! roles api,monitor privilege read/write");
    ...
    if (!strcmp(what, "COUNTER")) {
      ...
      if (argc < 3) {
        ...
        debug_println("  roles: api,cli,editor,monitor or 'all'");
        debug_println("  privilege: read, write, read/write (or rw)");
        debug_println("Example: set user api_user password ChangeMe123! roles api privilege read/write");
        return true;
      }
